...
/* La classe analog de Arduino offre plusieurs fonctionnalitÃ©s:
analogReadResolution(resolution): set the sample bits and resolution. It can be a value between 9 (0 â 511) and 12 bits (0 â 4095). Default is 12-bit resolution.
analogSetWidth(width): set the sample bits and resolution. It can be a value between 9 (0 â 511) and 12 bits (0 â 4095). Default is 12-bit resolution.
...
adcStart(pin), adcBusy(pin) and resultadcEnd(pin): starts an ADC convertion on attached pinâs bus. Check if conversion on the pinâs ADC bus is currently running (returns TRUE or FALSE). Get the result of the conversion: returns 16-bit integer.
 */
